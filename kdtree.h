...
class KDTree
{
  ...
  private:
     ...
     */
    double getDist(const Point<Dim>& check, const Point<Dim>& target) const;
    int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivot, int curDim);
    Point<Dim>& select(vector<Point<Dim>>& newPoints, int left, int right, int target, int curDim);
    void swap(vector<Point<Dim>>& newPoints, int one, int two);
    typename KDTree<Dim>::KDTreeNode* treeBuilder(vector<Point<Dim>>& newPoints, int left, int right, int curDim);
    void copy(KDTreeNode* self, KDTreeNode* other);
    void clear(KDTreeNode* subroot);
    Point<Dim> neighborHelper(KDTreeNode * subroot, const Point<Dim>& query, int curDim) const;
};
